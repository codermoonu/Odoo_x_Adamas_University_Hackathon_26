...
const CheckInButton = ({todayRecord, onAction}) => {
    ...
    if(todayRecord?.checkOut){
        return (
            <div className='flex flex-col items-center justify-center p-8 bg-slate-50 border-2 border-slate-700'>
  ...
  return (
    <div className='absolute bottom-4 right-4 flex flex-col z-1'>
        <button onClick={handleAttendance} disabled={loading} className={`w-full max-w-xs flex justify-between items-center gap-8 p-4 border-2 shadow-md text-white ${isCheckedIn ? "bg-[#1a1a1a] border-slate-700" : "bg-indigo-600 border-indigo-600"}`}>
